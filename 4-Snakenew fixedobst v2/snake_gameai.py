...
# Define constants
BLOCK_SIZE = 20
SPEED = 0  # Set to 0 for the fastest training
WHITE = (255, 255, 255)
...
class SnakeGameAI:
    ...
    def reset(self):
        ...
        self.score = 0
        self.food = None
        self.obstacles = self._create_fixed_obstacles()  # Fixed obstacles
        self._place_food()
    ...
    def play_step(self, action):
        ...
        if self.render:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    quit()
        # 2. Move
        prev_distance = self._distance(self.head, self.food)
        self._move(action)
        ...
        if self.is_collision():
            game_over = True
            reward = -20  # Increased penalty for collision
            return reward, game_over, self.score

        # 4. Calculate reward based on movement
